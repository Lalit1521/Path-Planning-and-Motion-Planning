...
#include "lalit_sridharan_roslab5/visualization.h"
#include <tf/tf.h>
  ...
  public:
    ...
    void handleLaserScan(const sensor_msgs::LaserScan::ConstPtr& msg) {
      ...
      if(prev_points.empty()){
        //ROS_INFO("First Scan");
        prev_points = points;
        //current_position = {0.275, 0.0, 0.0};
        return;
      }
      ...
      //errors = std::numeric_limits<double>::max();

      while (count < MAX_ITER && (curr_trans != prev_trans || count==0)) {
      //while (count < MAX_ITER && (errors > 10.0)) {
      //while (count < MAX_ITER){
        ...
        //ROS_INFO("check_complete");

        prev_trans = curr_trans;
        ++count;

        // **************************************** We update the transforms here ******************************************* ////
        //errors = updateTransform(corresponds, curr_trans);
         updateTransform(corresponds, curr_trans);
      }
      //ROS_INFO("pose: %f, %f, %f", curr_trans.x_disp, curr_trans.y_disp, curr_trans.theta_rot);
      ...
      ROS_INFO("Callback duration: %.6f seconds", duration.toSec());
    }
    ...
    void publishPos2()
    {
      ...
      */
      ...
      Eigen::Vector2f translation_vector;
      Eigen::Vector3d current_pose;
      Eigen::Vector3d previous_pose;
      previous_pose << current_position[0], current_position[1], current_position[2];
      translation_vector << curr_trans.x_disp, curr_trans.y_disp;
      Rotation_matrix << std::cos(current_position[2]), -std::sin(current_position[2]),
                         std::sin(current_position[2]), std::cos(current_position[2]);
      translation_vector =  Rotation_matrix * translation_vector;
      float theta = current_position[2] + curr_trans.theta_rot;
      current_position = {static_cast<float>(previous_pose(0) + translation_vector(0)),
                          static_cast<float>(previous_pose(1) + translation_vector(1)),
